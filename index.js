var S = { 
  init: function () {
    ...
    var hasSwitchedToCountdown = false;
    var liveCountdown = null;
    ...
    if (distance > 0) {
      ...
      setTimeout(function () {
        if (hasSwitchedToCountdown) return;
        hasSwitchedToCountdown = true;

        liveCountdown = setInterval(function () {
          var currentNow = new Date().getTime();
          var currentDistance = targetDate - currentNow;

          if (currentDistance <= 0) {
            clearInterval(liveCountdown);
            S.UI.simulate("|HAPPY|NEW|YEAR|2026|#rectangle|");
            return;
          }

          var secondsLeft = Math.floor(currentDistance / 1000);
          S.UI.simulate("|#countdown " + secondsLeft + "|");
